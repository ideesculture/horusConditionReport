		...
		public function hookPeriodicTask(&$pa_params) {
			...
			// Get new email
				...
				foreach ($o_mail as $vn_message_num => $o_message) {
					...
					$t_object->insert();
					DataMigrationUtils::postError($t_object, "While adding object", "horusConditionReportForCAPlugin"); // TODO: log this
					
					$t_object->addLabel(
						array('name' => $vs_subject), $pn_locale_id, null, true
					);
					DataMigrationUtils::postError($t_object, "While adding label", "horusConditionReportForCAPlugin");  // TODO: log this
					
					// Add representation
					$vs_tmp_file_path = tempnam(caGetTempDirPath(), 'horus_');
					foreach($va_images as $vn_i => $vs_file_content) {
						if (file_put_contents($vs_tmp_file_path, base64_decode((string)$vs_file_content))) {
							...
							DataMigrationUtils::postError($t_object, "While adding media", "horusConditionReportForCAPlugin");  // TODO: log this
						}
					}
